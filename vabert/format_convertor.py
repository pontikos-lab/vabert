"""Adopted from https://github.com/pranav-s/brat2CoNLL
"""
...
class FormatConvertor:
    def __init__(self, input_dir: str, output_file: str):
        #self.input_dir = '/Users/quangnguyen/Projects/Tools/brat/data/phenotype'
        #self.output_file = 'phenotypeCoNLL5.txt'
    ...
    def parse_text(self):
        ...
        with open(self.output_file, 'w') as fo:
            for file_count, file_pair in enumerate(file_pair_list):
                ...
                print(annotation_file, ' | ', text_file)
                annotation_count = 0
                ...
                current_index = 0               # follow text
                num_tokens = len(text_tokens)
                i = 0 # Initialize Token number; follow tokens
                
                double_newline = []
                for m in re.finditer('\n\n', text_string):
                    double_newline.append(m.start())
                iter_double_nl = iter(double_newline)
                current_double_nl = next(iter_double_nl)
                ...
                while i < num_tokens:
                    ...
                    if current_index != current_ann_start:
                        ...
                        while current_index <= current_ann_end and i < num_tokens:
                            print(text_tokens[i])
                            if is_B:
                                fo.write(f'{text_tokens[i]} B-{label}\n')
                                is_B = False
                            else:
                                fo.write(f'{text_tokens[i]} I-{label}\n')
                            current_index += len(text_tokens[i])+1
                            i += 1
                        is_B = True
                        annotation_count += 1
                    ...
                    if current_index >= current_double_nl:      # update current_index everytime passing a double newline
                        current_index += 1
                        try:
                            current_double_nl = next(iter_double_nl)
                        except StopIteration:
                            current_double_nl = len(text_string)
...
if __name__ == '__main__':
    ...
    #python brat2CoNLL/format_convertor.py --input_dir='../phenotype' --output_file='phenotypeCoNLL2.txt'